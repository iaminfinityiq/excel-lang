...
void print_expr(Expression *expr)
{
    ...
    case NodeType::CALL_EXPRESSION:
    {
        ...
        uint64_t arguments_size = call->arguments.size();
        for (uint64_t i = 0; i < arguments_size; i++)
        {
    ...
    default:
        std::cerr << "Intstruction not found" << std::endl;
    }
}

void print_stmt(const uint64_t indentation, Statement *stmt)
{
    for (uint64_t i = 0; i < indentation; i++)
    {
        std::cout << "  ";
    }
...
{
    for (uint64_t i = 0; i < interpolator->instructions.size(); i++)
    {
        ...
        for (uint64_t j = 0; j < interpolator->instructions[i].arguments.size(); j++)
        {
